...
geocell_ds = ogr.Open('./grid.gpkg')
	# This is a 1000x1000m grid in UTM15 (aligned to 3000x3000m)
	# smaller tiles to process higher density metro area
	# X:456000,525000 Y:4923000,5001000

tidx_lyr = tindex_ds.GetLayer('tindex');
geocell_lyr = geocell_ds.GetLayer('grid1k')
...
for geocell in geocell_lyr:
	...
	for t in tidx_lyr:
		...
		if (len(location) >= 78):
				# len(location) >= 78 selects the high res metro tiles
	...
	output_bn = './results.2m/' + geocell_name
